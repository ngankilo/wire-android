...
object ResString {
  ...
  def apply(resId: Int, quantity: Int, args: AnyRef*): ResString = ResString(resId, quantity, AnyRefArgs(args.toList))
  def apply(resId: Int, quantity: Int)               : ResString = ResString(resId, quantity, StringArgs(List(quantity.toString)))
