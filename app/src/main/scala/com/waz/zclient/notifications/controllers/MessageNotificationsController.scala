...
class MessageNotificationsController(bundleEnabled: Boolean = Build.VERSION.SDK_INT > Build.VERSION_CODES.M,
  ...
  private def createConvNotifications(userId: UserId, silent: Boolean, nots: Seq[NotificationInfo], teamName: Option[String]): Unit =
    if (nots.isEmpty) {
      verbose(s"a notification from userId is being cancelled: ${toNotificationGroupId(userId)}")
      notificationToCancel ! toNotificationGroupId(userId)
    }
    else {
      val groupedConvs =
        if (bundleEnabled)
      ...
      if (bundleEnabled) {
        ...
        inject[NotificationManagerWrapper].getActiveNotificationIds
          ...
          .foreach { c =>
            verbose(s"a notification from active notifications ids is being cancelled: $c")
            notificationToCancel ! c
          }

        val summaryId = toNotificationGroupId(userId)
        if (notIdSet.contains(summaryId)) {
          verbose(s"a summary notification is being cancelled: $summaryId")
          notificationToCancel ! summaryId
        }
